...
import time
import alpa
from alpa.utils import get_memory_usage, exp_recorder
...
from alpa.controller import Controller
import json
...
def parse_args():
    ...
    parser.add_argument("--alpa", action="store_true", help="Whether or not to use alpa")
    ...
    parser.add_argument("--device", type=int, default=0, help="hidden size")
    parser.add_argument('--bit', type=float, default=0, help='bit')
    ...
    if args.alpa:
        assert args.gradient_accumulation_steps == 1, "alpa works with accumulation step = 1"
...
def main():
    ...
    lr_scheduler = get_scheduler(
        ...
        optimizer=optimizer,
        num_warmup_steps=int(args.max_train_steps/10),
    ...
    # prepare alpa
    if args.alpa:
        alpa.set_config(bit=args.bit, total_step=args.max_train_steps)
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if args.get_mem and iter > 1:
                    ...
                    init_mem = get_memory_usage(False, args.device)  # model size + data size
                ...
                if args.get_mem and iter > 1:
                    ...
                    torch.cuda.synchronize()
                    before_backward = get_memory_usage(True, args.device)
                optimizer.zero_grad()
                loss.backward()
                ...
                if args.get_mem and iter > 1:
                    ...
                    for t in batch:
                        del t
                    after_backward = get_memory_usage(False, args.device)  # model size
