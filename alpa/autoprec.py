...
import numpy as np
from alpa.utils import exp_recorder
import alpa.cpp_extension.calc_precision as ext_calc_precision
from alpa.conf import config
from alpa.utils import exp_recorder
...
class AutoPrecision:
    def __init__(self, model, quantizer, bits, max_bits,
                 work_dir, adapt_step, log_interval,
    ...
    def iterate(self, backprop):
        def sample_grad():
            ...
            for param in self.model.parameters():
                if param.grad is not None:
                    param.grad[torch.isnan(param.grad) | torch.isinf(param.grad)] = 1e10 # deal if nan values exist                    
                    grad.append(param.grad.norm().unsqueeze(0)) # use parameter gradient normalization values instead of storing all parameters
        ...
        if self.iter == 0 or self.iter == self.adapt_step :
            # Do full adaptation
            print('ALPA: Initializing AutoPrec..., run extra %d iters' %
                      (len(self.quantizer.bits)))
            sensitivity = []
            bits = []
            # use different random seeds to update snesitivity
            for l in self.quantizer.bits:
                if not (config.fast_mem_check) or l==0:
                    b = self.quantizer.bits[l]
                    self.quantizer.bits[l] = 1
                    #param_grad_backward = get_memory_usage(True)
                    grad0 = get_grad()
    ...
    def refresh_bits(self):
        ...
        for l in self.dims:
            ...
            C_list.append(self.C[l])
            CD_list.append(self.C[l]/self.dims[l])
        ...
        if self.log_iter > 0 and self.iter > self.warmpup_iter:
            ...
            if quantization_var > overall_var * 0.1:
                print("========================================")
                print('ALPA Warning: Quantization variance is too large. Consider increasing number of bits.',
